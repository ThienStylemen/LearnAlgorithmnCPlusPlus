...
void Dictionary::DisplayAGame() {
	...
	while (left <= right) {
		int guessing_index = right + (left - right) / 2;
		cout << "computer say : is this your numeber : " << this->words[guessing_index];
		cout << endl << "your ans here : ";
		cin >> ans_of_user;
		if (ans_of_user == 0 ) {
			cout << "congratulations";
			return;
		}
		else if (ans_of_user == -1)  {
			right = guessing_index - 1;
		}
		else if (ans_of_user ==1) {
			left = guessing_index + 1;
		}
	}
}
...
int main() {
	...
	dtn.AddAWords("bmember");
	dtn.AddAWords("cmember");
	dtn.AddAWords("dmember");
	dtn.AddAWords("emember");
	dtn.AddAWords("fmember");
	dtn.AddAWords("gmember");
	dtn.AddAWords("hmember");
	dtn.AddAWords("jmember");
	dtn.AddAWords("kmember");
	dtn.Sorts();
	dtn.DisplayAGame();
